...
const Footprints = (props) => {
    ...
    const selectDestination = async (feature) => {
        ...
        try {
            ...
            const response = await axios.post('http://192.168.29.15:3000/api/v1/user/initialize-journey', { email: userdata.email, startPoint, endPoint });
    ...
    const fetchCurrentLocation = async () => {
        try {
            ...
            try {
                ...
                const response = await axios.post('http://192.168.29.15:3000/api/v1/user//set-footprint', { email: userdata.email, coordinates });
    ...
    const handleShareLinkwithOtherApps = async () => {
        try {
            const message = `https://www.google.com/maps/dir/?api=1&destination=${destinationCoords[0]},${destinationCoords[1]}`;
    ...
    return (
        <SafeAreaView style={{ height: '100%', backgroundColor: '#FFFFFF', padding: 4 }}>
            ...
            <HeaderTab />
            {
                !showMap && (<View style={{ backgroundColor: '#fff', zIndex: 1 }}>
                    <View style={{ paddingTop: 20 }}>
                        {/* Main prompt */}
                        <View style={{ marginBottom: 20 }}>
                            <Text style={{ fontSize: 24, color: '#000000', fontFamily: 'Ubuntu-Bold' }}>
                                {displayedText}
                            </Text>
                        </View>
